...
```js
fetch().then(res => {
  const data = res.data || [] // 提供默认数据
})
```

### 分号 / 引号

- 默认采用 `Standard` 规范，使用单引号，句末不添加分号，除了会导致语法错误时。

- `VSCdoe` 请 [参照此处](/frontend/VSCode.html#%E5%AE%89%E8%A3%85-prettier) 配置你的编辑器。

- `Vue` 项目 `Eslint` 配置中会配置 `Standard` 规范的严格模式。
